...
public class Views.AllTasks : Gtk.EventBox {
    ...
    construct {
        ...
        alert_view = new Widgets.AlertView (
            _("All clear"),
            _("Find all your tasks here."),
            "mail-mailbox-symbolic"
        );

        var all_tasks_icon = new Gtk.Image.from_icon_name ("user-bookmarks", Gtk.IconSize.DND);

        var all_tasks_label = new Gtk.Label ("<b>%s</b>".printf (_("All Tasks")));
        all_tasks_label.get_style_context ().add_class (Granite.STYLE_CLASS_H2_LABEL);
        all_tasks_label.use_markup = true;
