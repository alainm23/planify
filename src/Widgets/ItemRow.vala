...
public class Widgets.ItemRow : Gtk.ListBoxRow {
    ...
    private Gtk.Revealer date_preview_revealer;
    ...
    public bool item_selected {
        set {
            if (value) {
                main_grid.get_style_context ().add_class ("highlight");
            } else {
                main_grid.get_style_context ().remove_class ("highlight");
            }
        }
    }
        ...
        set {
            ...
            if (view == "today" || view == "upcoming" || view == "label") {
                if (view == "today" || view == "upcoming") {
                    date_preview_revealer.reveal_child = false;
                    time_preview_revealer.reveal_child = false;   
                }

                project = Planner.database.get_project_by_id (item.project_id);
                if (item.section_id != 0) {
                    var section = Planner.database.get_section_by_id (item.section_id);
                    project_preview_label.label = "<small>%s/%s</small>".printf (
                        Planner.utils.get_dialog_text (project.name),
                        Planner.utils.get_dialog_text (section.name));
                } else {
                    project_preview_label.label = "<small>%s</small>".printf (Planner.utils.get_dialog_text (project.name));
                }
                ...
                GLib.DateTime datetime = new GLib.DateTime.from_iso8601 (item.due_date, new GLib.TimeZone.local ());
                
                if (Planner.utils.is_overdue (datetime)) {
                    date_preview_revealer.reveal_child = true;
                    update_duedate_text ();
                }
                ...
                duedate_preview_revealer.reveal_child = time_preview_revealer.reveal_child || date_preview_revealer.reveal_child;
                check_preview_box ();
            } else {
    ...
    construct {
        ...
        content_entry.sensitive = item.checked == 0;
        if (item.checked == 0) {
        ...
        top_eventbox.expand = true;
        top_eventbox.above_child = false;
        ...
        date_preview_revealer.add (duedate_preview_label);
        date_preview_revealer.reveal_child = true;
        ...
        time_preview_label.valign = Gtk.Align.CENTER;
        ...
        reminder_preview_grid.margin_top = 1;
        ...
        note_preview_image.pixel_size = 9;
        ...
        labels_preview_box.valign = Gtk.Align.CENTER;
        ...
        checklist_button.tooltip_text = _("Add sub-task");
        ...
        completed_listbox.margin_start = 12;
        ...
        handle_grid.add (listbox_revealer);
        handle_grid.add (completed_revealer);
        ...
        checklist_button.clicked.connect (() => {
            var index = Planner.settings.get_enum ("new-tasks-position");
        ...
        Planner.database.item_parent_moved.connect ((i, parent_id, old_parent_id) => {
            Idle.add (() => {
                if (item.id == old_parent_id) {
                    foreach (unowned Gtk.Widget child in listbox.get_children ()) {
                        var row = (Widgets.ItemRow) child;
        ...
        Planner.database.add_due_item.connect ((i) => {
            if (item.id == i.id) {
                ...
                reschedule_button.update_duedate (i);
            }
        });
        ...
        Planner.database.item_uncompleted.connect ((i) => {
            Idle.add (() => {
                if (item.id == i.parent_id) {
                    if (items_completed_added.has_key (i.id.to_string ())) {
                        items_completed_added.unset (i.id.to_string ());
                    }

                    if (items_uncompleted_added.has_key (i.id.to_string ()) == false) {
                        var row = new Widgets.ItemRow (i);
            ...
            Idle.add (() => {
                if (item.id == i.parent_id) {
                    if (items_uncompleted_added.has_key (i.id.to_string ())) {
                        items_uncompleted_added.get (i.id.to_string ()).destroy ();
                        items_uncompleted_added.unset (i.id.to_string ());
                    }

                    if (items_completed_added.has_key (i.id.to_string ()) == false) {
                        var row = new Widgets.ItemRow (i);

                        items_completed_added.set (i.id.to_string (), row);
    ...
    private void update_project_preview_label () {
        project = Planner.database.get_project_by_id (item.project_id);
    ...
    private void check_preview_box () {
        if (null != bottom_revealer && bottom_revealer.reveal_child == false) {
            preview_revealer.reveal_child = date_preview_revealer.reveal_child ||
            time_preview_revealer.reveal_child ||
    ...
    private void add_all_checks () {
        foreach (var i in Planner.database.get_all_cheks_by_item (item.id)) {
            var row = new Widgets.ItemRow (i);
    ...
    private void add_completed_items () {
        items_completed_added.clear ();
        foreach (var i in Planner.database.get_all_completed_items_by_parent (item.id)) {
            var row = new Widgets.ItemRow (i);
    ...
    private void update_duedate_text () {
        ...
        } else if (item.checked == 1 && item.date_completed != "") {
            duedate_preview_grid.margin_start = 1;
            var datetime = new GLib.DateTime.from_iso8601 (item.date_completed, new GLib.TimeZone.local ());
            duedate_preview_label.label = "<small>%s</small>".printf (
                Planner.utils.get_relative_date_from_date (datetime)
            );
            duedate_preview_label.get_style_context ().add_class ("completed-label");
            duedate_preview_label.get_style_context ().add_class ("font-bold");
            date_preview_revealer.reveal_child = true;
        }
    ...
    private void build_context_menu (Objects.Item item) {
        ...
        move_section_menu = new Widgets.ImageMenuItem (_("Move to Section"), "section-symbolic");
        ...
        today_menu.activate.connect (() => {
            reschedule_button.set_new_item_due_date (
                Planner.utils.get_datetime (
                    new GLib.DateTime.now_local ()
                )
            );
            reschedule_button.save_item ();
        });
