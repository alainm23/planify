...
class ToastFixture {
  ...
  async screenshot(screenshotModifier, el) {
    const { page } = this;
  ...
  skipRTL(testRef, reason = 'This functionality does not have RTL-specific behaviors.') {
    const { testInfo } = this;
    testRef.skip(testInfo.project.metadata.rtl === true, reason);
  }
  skipMode(testRef, mode, reason) {
    const { testInfo } = this;
