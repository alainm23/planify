...
test.describe('modal: inline', () => {
  ...
  test('presenting should create a single root element with the ion-page class', async ({ page, skip }, testInfo) => {
    ...
    expect(await modal.evaluate((el) => el.firstElementChild.className)).toContain('ion-page');
    await modal.evaluate((el) => el.dismiss());
    await ionModalDidDismiss.next();
    await page.click('#date-button');
    await ionModalDidPresent.next();
    ...
    expect(await modal.evaluate((el) => el.firstElementChild.className)).toContain('ion-page');
    expect(await modal.evaluate((el) => el.firstElementChild.firstElementChild.className)).not.toContain('ion-page');
  });
});
